...
instance Input Process where
    ...
    getRaw (Process path args) = liftIO . fmap (\(_, out, _) -> fromStrict out) $ readProcessWithExitCode (toSL path) (toSL <$> args) mempty
...
instance Output Process where
    ...
    putRaw (Process path args) d = liftIO . void . readProcessWithExitCode (toSL path) (toSL <$> args) $ toStrict d
